...
// Create a simplified placeholder component that shows while the gallery is loading
const GalleryPlaceholder = () => (
  <LazyMotion features={domAnimation}>
    <div className="w-full h-[calc(100vw*0.95)] sm:h-[calc(100vw*0.75)] md:h-[calc(100vw*0.65)] lg:h-[calc(100vw*0.55)] xl:h-[calc(100vw*0.45)] 2xl:h-[calc(100vw*0.4)] max-h-[80vh] relative flex items-center justify-center bg-[#29552a]">
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <m.div 
          ...
          className="text-white font-lora text-lg sm:text-xl md:text-2xl font-medium"
        >
          Loading Gallery...
        </m.div>
        <m.div 
      ...
      <div className="absolute inset-0 flex justify-center items-center pointer-events-none">
        <div className="flex space-x-4 sm:space-x-8 md:space-x-12 opacity-30">
          {[1, 2, 3].map((i) => (
            <div key={i} className="w-32 sm:w-40 md:w-52 lg:w-60 aspect-[9/16] bg-black/30 rounded-md"></div>
          ))}
        </div>
      </div>
    </div>
